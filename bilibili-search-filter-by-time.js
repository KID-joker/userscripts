...
(function () {
	...
	async function requestData(url, options) {
		options.timeout = 20000; // 不管用，在拦截fetch之前已经用这个参数了
		let timeoutFLag = false;
		setTimeout(()=>{
			timeoutFLag = true; // 防止超时
		}, 7000);
		...
		while (true) {
			...
			if (_responseJson.data && _responseJson.data.result) {
				debugger
				if (_responseJson.data.result.length < pageSize) { // 42
					finished = true;
					maxPage = actualPage;
				}
			...
			requestPage++;
			if (timeoutFLag || finished || result.length >= actualPage * actualPageSize) {
				return;
			} else {
